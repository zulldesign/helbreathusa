...
public partial class UserControls_GoogleCheckoutControl : System.Web.UI.UserControl
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!Page.IsPostBack)
        {
            if (StoreConfiguration.GetConfigurationValue(ConfigurationKey.GoogleImageButtonURL) != null && StoreConfiguration.GetConfigurationValue(ConfigurationKey.GoogleMerchantID) != null)
    ...
    {
        var shippingCollection = CacheManager.GetCachedLookupTable(LookupDataEnum.GetShippingPoviders);
        ...
        int currentItem = 0;
        foreach (var item in shippingCollection)
        {
            if (string.IsNullOrEmpty(item.Value))
                continue;
            FlatRateShipping rate = new FlatRateShipping();
            rate.name = item.Value;
            rate.price = new FlatRateShippingPrice() { currency = "USD", Value = Orders.GetShippingCosts(item.Key) };
